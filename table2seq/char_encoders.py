...
class T2STokenEncoder(nn.Module):
    ...
    def word_char_fusion(cls, word_embed, sub_chars, word_embed_matrix):
        char_seq, char_seq_len, dynamic_char_vocab, dynamic_char_length = sub_chars
        # => (batch, sub_seq_len)
        vocab_size, sub_seq_len = dynamic_char_vocab.shape
        # => Time First
        # char_seq = char_seq.permute(1, 0, 2)
        # char_len = char_len.permute(1, 0)
        # => (batch*seq, sub_seq, dim)
        ...
        return  cls.output_projection(torch.cat([word_embed,char_embed], -1))
