    ...
    def process(self, batch, device=None):
        ...
        for bid in range(len(uni_token_char_list)):
            for tid, char in enumerate(uni_token_char_list[bid]):
                uni_token_char_list[bid][tid] = self.vocab.stoi[char]
        dynamic_vocab = torch.tensor(uni_token_char_list, dtype=self.dtype, device=device)
        uni_token_length = torch.tensor(uni_token_length, dtype=self.dtype, device=device)
        return var, lengths, dynamic_vocab,uni_token_length
